...
router.get('/', (req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      'POST /upload/image - 上传图片到S3 (需要认证)',
      'POST /upload/payment-qr - 上传支付二维码到S3',
      'POST /upload/transfer-screenshot - 上传转账截图到S3',
      'DELETE /upload/:filename - 删除S3文件 (需要认证)',
      'GET /upload/environment-info - 获取S3环境信息 (需要认证)',
